  ...
  return (
    <>
      {main && (
        <Link
          href={button}
          className="bg-white text-black px-6 h-14 text-2xl lg:text-base tracking-wider flex flex-row font-black gap-3 w-fit items-center rounded hover:bg-opacity-60 ease-in duration-300"
          target="_blank"
        >
      ...
      {!main && (
        <Link
          ...
          target="_blank"
        >
          <div className="w-10 h-10 lg:w-6 lg:h-6">
            {type == "Github" && <AiFillGithub className="w-full h-full" />}
            {type == "Youtube" && <AiFillYoutube className="w-full h-full" />}
            {type == "External" && <BiLinkExternal className="w-full h-full" />}
          </div>
